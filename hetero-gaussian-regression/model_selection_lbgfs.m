...
if optmethod ==1
    options = optimoptions('fmincon','Algorithm','interior-point',...
        ...
        'MaxIterations', 40);
    lb = [-5*ones(size(sigma)); 3*ones(size(l)); -5*ones(size(seps))]; %lower bound 
...
function [f, g] = evidence(X, obi, nf, filledValue, hetsig, hetl, hetseps)
    ...
    for i = 1:filled
        for j = 1:filled
            ...
            cov(i,j) = sigma(i)*sigma(j)*sqrt(2*l(i)*l(j)/(l(i)^2+l(j)^2))...
                                      *exp(-dist/(l(i)^2+l(j)^2));
%             cov2(i,j) = cov(i,j)*((-2*l(j)*l(i)^2 + 2*l(j)^3)/...
%                  ((l(i)^2+l(j)^2)*l(i)*l(j))+4*l(i)*dist/(l(i)^2+l(j)^2)^2);
            cov2(i,j) = 2*cov(i,j)*((l(i)^2+l(j)^2)/(2*l(i)*l(j))*(l(j)^3 - l(i)^2*l(j))+2*l(i)*dist)/(l(i)^2+l(j)^2)^2;
        end
    end
    K = cov + diag(seps.^2);
    try
