...
l = 0.0007;
h = sqrt(1/l);
seps = 0.2*ones(filled,1); %\sigma_\eps
logp = -inf;
nobi = length(obi); %number of observed entry 
cov = zeros(nobi, nobi);
cov2 = zeros(nobi, nobi); %this covariance matrix is for gradient calculation
...
while cycle < 1
    ...
    while itr<200
        ...
        aat = alpha*alpha';
        invK = inv_chol(K);
%         invK = solve_chol(L, eye(length(L)));
        ami = aat - invK;
...
% this is very important, without this performance will be worse
